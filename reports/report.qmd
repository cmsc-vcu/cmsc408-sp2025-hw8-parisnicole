...
region_income_counts AS (
  SELECT 
    ...
    CAST(COUNT(*) AS DECIMAL(4,1)) AS total_countries
  FROM 
    wdi_country
  GROUP BY 
    `Region`
...
SELECT 
  r.`Region`,
  COALESCE(c.high_income, 0) AS high_income,
  COALESCE(c.upper_middle_income, 0) AS upper_middle_income,
  COALESCE(c.lower_middle_income, 0) AS lower_middle_income,
  COALESCE(c.low_income, 0) AS low_income,
  COALESCE(c.other, 0) AS other,
  COALESCE(c.total_countries, 0) AS total_countries
FROM 
  all_regions r
LEFT JOIN 
  region_income_counts c ON r.`Region` = c.`Region`
ORDER BY 
  r.`Region`;
""")
...
SELECT 
  ac.Region, 
  ac.income_group,
  0 AS country_count  -- this is the key fix!
FROM 
  all_combinations ac
LEFT JOIN 
...
WITH income_counts AS (
  SELECT 
    COALESCE(`Income Group`, 'Unknown') AS income_group,
    COUNT(*) AS country_count
  FROM 
    wdi_country
  GROUP BY 
